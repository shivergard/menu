...
abstract class PackageController extends BaseController {
	...
	public function __construct()
	{
		if (class_exists('App\Model\Akwilon\Roles')){
			$this->validateRole(\App\Model\Akwilon\Roles::where('name' ,'menu'));
		}else if (class_exists('App\User\Roles')){
			$this->validateRole(\App\User\Roles::where('name' ,'menu'));
		} else if (!is_object(Auth::user()) || !Auth::user()->name == 'menu'){
			Redirect::to('/')->send();
		}
	}
